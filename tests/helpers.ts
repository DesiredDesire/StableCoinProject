...
export const addPairWithAmount = async (pair: KeyringPair, amount: BN): Promise<KeyringPair> => {
  ...
  await buildTx(api.registry, api.tx.balances.transfer(redspotPair.address, amount), (await getAddresses())[0]);
  return redspotPair;
};

export const getSigner = async (account: string) => {
  // eslint-disable-next-line @typescript-eslint/no-magic-numbers
  const signer = await addPairWithAmount(new Keyring().addFromUri(`//${account}`), one.muln(10000));
  return signer;
};
