...
impl<T: EatingStorage + OwnableStorage> Eating for T {
    default fn eat_collateral_price(&self) -> Result<u128, EatingError> {
        match FeedingRef::feed_price(&EatingStorage::get(self).vault_feeder_address) {
            Ok(v) => Ok(v),
            Err(e) => Err(EatingError::from(e)),
        }
    }

    default fn change_feeder(&mut self, new_vault_feeder_address: AccountId) -> Result<(), EatingError> {
        let old_feeder = EatingStorage::get(self).vault_feeder_address;
        EatingStorage::get_mut(self).vault_feeder_address = new_vault_feeder_address;
